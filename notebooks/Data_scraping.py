...
#Scraped : machine learning, android app development, data-science, artificial intelligence, web development, backend development
existing_df = pd.read_csv("Z:\Python\Internship_Recommender\data\internships_cleaned.csv")

base_url = "https://internshala.com/internships/backend-development-internship"

# for page in range(1, 9): #Change the pages!
#     url = f"{base_url}/page-{page}"
#     print("Scraping:", url)
#     res = requests.get(url, headers={"User-Agent": "Mozilla/5.0"})
#     soup = BeautifulSoup(res.text, 'html.parser')
...
#NEW LOOP FOR FIXING THE SKILLS SPACEING ISSUE
for page in range(1, 9):
    url = f"{base_url}/page-{page}"
    print("Scraping:", url)
    res = requests.get(url, headers={"User-Agent": "Mozilla/5.0"})
    ...
    for intern in internships:
        ...
        # ✅ FIX HERE
        skills_tags = intern.find_all('span', class_='round_tabs')
        skill_list = [s.text.strip() for s in skills_tags]
...
#Appending the new scraped data to old one
updated_df = pd.concat([existing_df, new_df], ignore_index=True)
updated_df.to_csv("Z:\Python\Internship_Recommender\data\internships_NEW.csv", index=False)
print("✅ Scraping complete! Data saved to internships.csv")